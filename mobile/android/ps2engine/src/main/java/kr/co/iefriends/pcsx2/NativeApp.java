...
public class NativeApp {
	...
	static {
		...
		try {
			...
			nativeLoadError = e.getMessage() == null ? e.getClass().getSimpleName() : e.getMessage();
			System.err.println("PCSX2_LOAD_FAILED " + libraryName + ": " + nativeLoadError);
		}
	}
	...
	public static void initializeOnce(Context context) {
		...
		File dataRoot = new File(app.getFilesDir(), "pcsx2");
		File biosRoot = new File(dataRoot, "bios");
		dataRoot.mkdirs();
		biosRoot.mkdirs();
		initialize(dataRoot.getAbsolutePath(), biosRoot.getAbsolutePath(), Build.VERSION.SDK_INT);
	}
	...
	public static void onPadRumble(int pad, int largeMotor, int smallMotor) {
		...
		int devId = nthGamepadDeviceId(pad);
	...
	public static void testRumble(int port) {
		int devId = nthGamepadDeviceId(port);
	...
	private static volatile boolean sVmPaused;
	public static boolean isVmPausedByCore() { return sVmPaused; }
